...
export default function EventLog({ events, connected }: EventLogProps) {
	return (
		<div className="h-48 bg-sefs-surface border-t border-sefs-border">
			<div className="flex items-center justify-between px-4 py-1.5 border-b border-sefs-border/50">
				<div className="flex items-center gap-2">
					<span className="text-xs font-medium text-sefs-muted">Event Log</span>
					<span
						className={`w-1.5 h-1.5 rounded-full ${
							connected ? "bg-sefs-success" : "bg-sefs-error"
						}`}
					/>
				</div>
				<span className="text-xs text-sefs-muted">{events.length} events</span>
			</div>
			<div className="overflow-auto h-[calc(100%-28px)] px-4 py-1">
				{events.length === 0 && (
					<div className="text-xs text-sefs-muted py-2">
						Waiting for events...
					</div>
				)}
				{events.slice(0, 50).map((event, i) => (
					<div key={i} className="flex items-center gap-2 py-0.5 text-xs">
						...
						<span className="text-sefs-muted">{formatEvent(event)}</span>
					</div>
				))}
			</div>
		</div>
